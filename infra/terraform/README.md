# Underlytics Terraform Bootstrap

This directory provisions the post-bootstrap GCP infrastructure described in `bootstrap.md`.
...
- Cloud Storage + Cloud CDN frontend skeleton
- Workload Identity Federation for GitHub Actions

## Important constraint

The current frontend is a Clerk-protected Next.js application that uses middleware,
server components, and route handlers. That architecture is **not directly deployable**
to Cloud Storage + Cloud CDN as a static site without additional frontend changes.

This Terraform still provisions the frontend bucket/CDN skeleton from `bootstrap.md`,
but the current application should be treated as **backend/infrastructure ready** rather
than fully frontend-deployable to static hosting.
...
## Frontend deployment caveat

The frontend bucket and CDN resources are provisioned because `bootstrap.md` requires them.
They should currently be treated as infrastructure placeholders only. The existing Clerk-based
Next.js app still needs SSR-capable hosting if you want production deployment without changing
